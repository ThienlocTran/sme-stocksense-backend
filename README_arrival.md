...
- **Database Migration**:
  - `src/main/resources/db/migration/V9__add_actual_arrival_date.sql`: Tạo cột `ngay_hang_ve` (timestamp) trong bảng `phieu_nhap_kho` để lưu ngày hàng về thực tế của phiếu nhập.
- **Thực thể (Entity)**:
