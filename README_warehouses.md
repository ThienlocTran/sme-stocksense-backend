...
- **Nghiệp vụ (Service)**:
  - `src/main/java/com/smartflow/smestocksensebackend/service/WarehouseService.java`: Định nghĩa interface dịch vụ bao gồm `getWarehouses` và `createWarehouse`.
  - `src/main/java/com/smartflow/smestocksensebackend/service/impl/WarehouseServiceImpl.java`: Logic tìm kiếm động, xử lý tạo mới kho hàng, validate mã kho không trùng và chuẩn hóa trạng thái/địa chỉ.
- **Điều phối (Controller)**:
  - `src/main/java/com/smartflow/smestocksensebackend/controller/WarehouseController.java`: API Endpoint đón nhận request `GET /api/warehouses` và `POST /api/warehouses`.
...
- **Kiểm thử (Unit Test)**:
  - `src/test/java/com/smartflow/smestocksensebackend/service/impl/WarehouseServiceImplTest.java`: Bổ sung kiểm thử đơn vị cho cả luồng lấy danh sách và tạo mới kho hàng.
